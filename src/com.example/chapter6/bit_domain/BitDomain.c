...
//定义简单的结构体
struct {
    unsigned int widthValidated;  //4字节
    unsigned int heightValidated; //4字节
} status1;

//这种结构需要 8 字节的内存空间，但在实际上，在每个变量中，我们只存储 0 或 1。在这种情况下，
//C 语言提供了一种更好的利用内存空间的方式。如果您在结构内使用这样的变量，您可以定义变量的宽度来告诉编译器，您将只使用这些字节。例如，上面的结构可以重写成：
...
//现在，上面的结构中，status2 变量将占用 4 个字节的内存空间，但是只有 2 位被用来存储值。
//如果您用了 32 个变量，每一个变量宽度为 1 位，那么 status2 结构将使用 4 个字节，但只要您再多用一个变量，
